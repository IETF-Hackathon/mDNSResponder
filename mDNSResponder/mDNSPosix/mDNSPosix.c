                                            ...
                                            domainname *hostname, mDNSBool useBackgroundTrafficClass)
{
    ...
    sock->events.fd = -1;
    if (!mDNSPosixTCPSocketSetup(&sock->events.fd, addrType, port))
    {
...
mDNSexport void *mDNSPlatformMemAllocateClear(const char *name,
                                              mDNSu32 len) { (void)name; // unused
                                                             return(callocL(name, len)); }
...
mDNSlocal void mDNSPosixGetFDSetWorker(mDNS *m, int *nfds, fd_set *readfds, fd_set *writefds)
{
    ...
    if (m->p->unicastSocket4 != -1) mDNSPosixAddToFDSet(&fdm, readfds, m->p->unicastSocket4);
#if HAVE_IPV6
    if (m->p->unicastSocket6 != -1) mDNSPosixAddToFDSet(&fdm, readfds, m->p->unicastSocket6);
#endif
    while (info)
    {
        if (info->multicastSocket4 != -1) mDNSPosixAddToFDSet(&fdm, readfds, info->multicastSocket4);
#if HAVE_IPV6
        if (info->multicastSocket6 != -1) mDNSPosixAddToFDSet(&fdm, readfds, info->multicastSocket6);
#endif
