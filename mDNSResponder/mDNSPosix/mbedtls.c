...
mDNSPosixTLSInit(void)
{
    ...
    if (status != 0) {
        LogMsg("Unable to seed RNG: %x", -status);
        return mDNSfalse;
    }
    return mDNStrue;
}
...
mDNSPosixTLSClientStateCreate(TCPSocket *sock)
{
    ...
    if (status != 0) {
        LogMsg("Unable to set ssl config defaults: %d", -status);
        return NULL;
    }
    ...
    tls = mDNSPlatformMemAllocateClear("PosixTLSClientStateCreate: TLSContext", sizeof *tls);
    if (tls == mDNSNULL)
        return tls;
...
{
    int status;
    TLSContext *tls = mDNSPlatformMemAllocateClear("PosixTLSAccept: TLSContext", sizeof *tls);
