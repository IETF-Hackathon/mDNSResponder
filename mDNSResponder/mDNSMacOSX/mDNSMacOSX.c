...
mDNSlocal mStatus SetupTCPSocket(TCPSocket *sock, mDNSAddr_Type addrtype, mDNSIPPort *port, mDNSBool useBackgroundTrafficClass)
{
    ...
    skt = -1;
    if (!mDNSPosixTCPSocketSetup(&skt, addrtype, port))
    {
        if (skt != -1) close(skt);
        return mStatus_UnknownErr;
    }
...
mDNSexport void *mDNSPlatformMemAllocateClear(const __unused char *info,
                                              mDNSu32 len) { return(callocL("mDNSPlatformMemAllocateClear", len)); }
